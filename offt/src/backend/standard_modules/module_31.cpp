...
/*
	Number of additions       : 816
	Number of multiplications : 160
*/

template<> StandardModuleComplexity const StandardModule<float, 31>::Complexity = { 816, 160 };
template<> StandardModuleComplexity const StandardModule<double, 31>::Complexity = { 816, 160 };
...
static void ComputeCore(valueT *pReal, valueT *pImag, ptrdiff_t stride, valueT const *twiddles)
{
	...
	ModuleBase<valueT>::ComplexMultiply(t48, t49, pReal[24 * stride], pImag[24 * stride], twiddles[48], twiddles[49]);
	ModuleBase<valueT>::ComplexMultiply(t50, t51, pReal[25 * stride], pImag[25 * stride], twiddles[50], twiddles[51]);
	ModuleBase<valueT>::ComplexMultiply(t52, t53, pReal[26 * stride], pImag[26 * stride], twiddles[52], twiddles[53]);
	ModuleBase<valueT>::ComplexMultiply(t54, t55, pReal[27 * stride], pImag[27 * stride], twiddles[54], twiddles[55]);
	ModuleBase<valueT>::ComplexMultiply(t56, t57, pReal[28 * stride], pImag[28 * stride], twiddles[56], twiddles[57]);
	ModuleBase<valueT>::ComplexMultiply(t58, t59, pReal[29 * stride], pImag[29 * stride], twiddles[58], twiddles[59]);
	ModuleBase<valueT>::ComplexMultiply(t60, t61, pReal[30 * stride], pImag[30 * stride], twiddles[60], twiddles[61]);
...
template<> void StandardModule<float, 31>::Compute(float *pReal, float *pImag, ptrdiff_t stride, float const *twiddles) const
{
	ComputeCore(pReal, pImag, stride, twiddles);
}

template<> void StandardModule<double, 31>::Compute(double *pReal, double *pImag, ptrdiff_t stride, double const *twiddles) const
{
	ComputeCore(pReal, pImag, stride, twiddles);
}
